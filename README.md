...
All supported Java types can be used as globals, retrieved as globals or used as function parameters or return values and map values. So nested objects and arrays are supported.
...
| `java.lang.Iterable<?>`                                     | `array`                 | Iterable (if not a QuickJSArray) is copied value by value to JS array. If extracted back from JS, the array will always return as a `com.github.stefanrichterhuber.quickjs.QuickJSArray`. Values can be of any of the supported Java types. |
