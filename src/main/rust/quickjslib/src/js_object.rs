...
where
    for<'js> F: FnOnce(&mut JNIEnv<'java>, Ctx<'js>, Object<'js>) -> R,
    R: 'java,
{
...
>(
    ...
    let result = with_object(_env, object_ptr, &ctx, |mut _env, _ctx, object| {
        object.len() as jint
    });
    result
}
...
pub extern "system" fn Java_com_github_stefanrichterhuber_quickjs_QuickJSObject_setValue<'a>(
    ...
    with_object(env, object_ptr, &ctx, |mut _env, _ctx, object| {
...
pub extern "system" fn Java_com_github_stefanrichterhuber_quickjs_QuickJSObject_getValue<'a>(
    ...
    let key = java_js_proxy::ProxiedJavaValue::from_object(&mut env, &ctx, key);
    let value = with_object(env, object_ptr, &ctx, |mut env, _ctx, object| {
...
pub extern "system" fn Java_com_github_stefanrichterhuber_quickjs_QuickJSObject_keySet<'vm>(
    ...
    let value = with_object(env, object_ptr, &ctx, |env, _ctx, object| {
        let object_keys: ObjectKeysIter<'_, JSJavaProxy<'_>> = object.keys();
        let java_set = js_java_proxy::create_java_set_from_object_keys_iter(&ctx, env, object_keys);
